...
function updateSensors(wink, row) {
	...
	document.getElementById("winkResult").innerHTML = "Found " + controlWinks.length + " Wink devices"; // + resp;
}
	...
	xhr.onreadystatechange = function() {
		if (this.readyState == 4)
			if (xhr.status != 200) {
				...
				document.getElementById("winkResult").innerHTML = "Error Calling Wink REST API " + xhr.status + " " + xhr.statusText;
				return;
			} else {
...
function setDevice(model, udn, value) {
	...
	req.onreadystatechange = function() {
		...
		if (req.status != 200) {
			// Handle request failure here...
			document.getElementById("winkResult").innerHTML = "Error Calling Wink REST API " + req.status;
			return;
		}
		// Request successful, read the response
		var resp = req.responseText;
		document.getElementById("winkResult").innerHTML = "SetDevice " + udn + " to " + value + ": " + resp;
		fetchDevices();
	}
